// src/services/fengShuiService.ts (ví dụ)
import { GoogleGenAI } from "@google/genai";
import { FengShuiInput, FengShuiResult } from "../types";

// Vite + Vercel: ENV dùng import.meta.env và PHẢI bắt đầu bằng VITE_
const GEMINI_API_KEY = import.meta.env.VITE_GEMINI_API_KEY as string;

export async function getAIFengShuiSummary(
  input: FengShuiInput,
  result: FengShuiResult
): Promise<string> {
  if (!GEMINI_API_KEY) {
    console.error("Thiếu VITE_GEMINI_API_KEY trong Environment Variables của Vercel.");
    return "Hiện tại hệ thống chưa được cấu hình khóa API. Vui lòng thử lại sau.";
  }
  ...
  const prompt = `
    ...
    Hãy viết một đoạn tóm tắt ngắn (khoảng 150 chữ) mang tính chuyên nghiệp và truyền cảm hứng
    về cách tối ưu hóa nội thất để kiến tạo một không gian sống hạnh phúc.
  ...
  try {
    ...
    // Tùy phiên bản SDK, có thể là response.text hoặc response.candidates[0].content
    const text = (response as any).text ?? (response as any)?.candidates?.[0]?.content?.parts?.[0]?.text;

    return (
      text ||
      "Chúc bạn kiến tạo được không gian sống hạnh phúc và tràn đầy năng lượng."
    );
  } catch (error) {
